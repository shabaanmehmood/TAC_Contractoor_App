...
// jobDetailsScreen.dart
class ContractorActiveJobDetailsScreen extends StatefulWidget {
  final MyjobsModel job;
  ContractorActiveJobDetailsScreen({super.key, required this.job});

  @override
  State createState() => _ContractorActiveJobDetailsScreenState();
}

class _ContractorActiveJobDetailsScreenState extends State<ContractorActiveJobDetailsScreen> {
  final List tabs = ["Details", "Shifts", "Guards", "Timeline"];
  int selectedIndex = 0;

  Widget getSelectedWidget() {
    switch (selectedIndex) {
      case 0:
        return activeDetailsWidget(widget.job);
      case 1:
        return activeShiftCard(widget.job);
      case 2:
        return activeGuards(); // Guards implementation ignored as per your instruction
      case 3:
        return activeTimeline(); // Timeline implementation can remain static or be updated if you have data
      default:
        return activeDetailsWidget(widget.job);
    }
  }
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text("Job Details", style: AppTypography.kBold20.copyWith(color: AppColors.kWhite)),
                ],
              ),
            ),
            // Divider
            ...
            Padding(
              padding: EdgeInsets.symmetric(horizontal: Get.height * 0.02, vertical: Get.height * 0.02),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.end,
                children: [
                  // Job ID skipped as per your instruction
                  // _buildStatusChip(widget.job.jobStatus?.toUpperCase() ?? "", AppColors.kblueCard.withOpacity(0.5), AppColors.kblueCard),
                  _buildStatusChip(widget.job.jobStatus?.toUpperCase() ?? "", AppColors.kblueCard.withOpacity(0.5), AppColors.kblueCard),
                ],
              ),
            ),
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        SizedBox(width: 5),
                        Text(
                          widget.job.shifts.isNotEmpty ? widget.job.shifts.first.date : '',
                          style: AppTypography.kLight14.copyWith(color: Color.fromARGB(255, 180, 189, 209)),
                        ),
                      ],
                    ),
                    SizedBox(height: 8),
