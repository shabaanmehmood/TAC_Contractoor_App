...
class _AdvancedFiltersViewState extends State<AdvancedFiltersView> {
  ...
  bool usePayRateFilter = false; // Toggle for pay rate filter
  String selectedJobType = "";
  String selectedShift = "";
  String selectedLevel = "";
  String selectedExperience = "";
  ...
  void initState() {
    ...
    if (Get.isRegistered<SearchViewController>()) {
      ...
      usePayRateFilter = searchController.usePayRateFilter.value;
      selectedJobType = searchController.jobType.value;
      selectedShift = searchController.shiftTiming.value;
      selectedLevel = searchController.requiredLevel.value;
      selectedExperience = searchController.experience.value;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildToggleSwitch('Use Current Location'),
              SizedBox(height: AppSpacing.fifteenVertical,),
              _buildToggleSwitch('Use Pay Rate Filter', isPayRateToggle: true),
              SizedBox(height: AppSpacing.tenVertical,),
              if (usePayRateFilter) ...[
                _buildDualSlider("Pay rate", "\$${_payRateMin.toStringAsFixed(1)} - \$${_payRateMax.toStringAsFixed(1)}"),
                SizedBox(height: AppSpacing.fifteenVertical,),
              ],
              _buildSelectableChips("Job Type", ["Full Time", "One Time", "Part Time"], selectedJobType, (val) {
                setState(() => selectedJobType = val);
              ...
              _buildMultiSelectChips("Licenses", ["Armed", "Construction Site", "Mining Site", "Asset Protection", "First Aid", "Driving License"], selectedLicenses),
              SizedBox(height: AppSpacing.fifteenVertical,),
              _buildMultiSelectChips("Premises / Site type", ["Event Security", "Corporate Office", "Mall Security", "OnSite"], selectedPremises),
              SizedBox(height: AppSpacing.fifteenVertical,),
              Row(
                children: [
                  Expanded(
  ...
  Widget _buildToggleSwitch(String title, {bool isPayRateToggle = false}) {
    return Container(
      padding: EdgeInsets.only(top: 5, bottom: 5, left: 15, right: 15),
      decoration: BoxDecoration(
        color: Color.fromRGBO(31, 41, 55, 1)
      ...
      child: Row(
        ...
        children: [
          ...
          Switch(
            thumbColor: WidgetStatePropertyAll(Colors.black),
            value: isPayRateToggle ? usePayRateFilter : useCurrentLocation,
            onChanged: (value) => setState(() {
              if (isPayRateToggle) {
                usePayRateFilter = value;
              } else {
  ...
  Widget _buildDualSlider(String title, String value) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(title, style: AppTypography.kBold16.copyWith(
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              SizedBox(height: 10),
              // Min value slider
              Text("Minimum: \$${_payRateMin.toStringAsFixed(1)}", 
              ...
              SizedBox(height: 10),
              // Max value slider
              Text("Maximum: \$${_payRateMax.toStringAsFixed(1)}", 
                style: AppTypography.kBold14.copyWith(color: AppColors.kSkyBlue)),
              Slider(
                value: _payRateMax,
                min: 0,
                ...
                onChanged: (value) => setState(() {
                  ...
                  if (_payRateMax <= _payRateMin) {
                    _payRateMin = _payRateMax - 10;
                    if (_payRateMin < 0) _payRateMin = 0;
                  }
                }),
                activeColor: AppColors.kSkyBlue,
                inactiveColor: Colors.grey,
  ...
  void _applyFilters() {
    ...
    Map<String, dynamic> filterData = {
      ...
      'usePayRateFilter': usePayRateFilter,
      'jobType': selectedJobType,
      'shiftTiming': selectedShift,
      'requiredLevel': selectedLevel,
      'experience': selectedExperience,
