...
import 'package:taccontractor/modules/shift_detail/inProgress/inProgress_components.dart/attendance.dart';
import 'package:taccontractor/modules/shift_detail/inProgress/inProgress_components.dart/notes.dart';
import 'package:taccontractor/modules/shift_detail/shift_detail_components/approve_clock_in.dart';
...
class InProgressShiftDetails extends StatefulWidget {
  const InProgressShiftDetails({super.key});

  @override
  State<InProgressShiftDetails> createState() => _InProgressShiftDetailsState();
}

class _InProgressShiftDetailsState extends State<InProgressShiftDetails> {
                    ...
                    Container(
                      ...
                      child: OutlinedButton(
                          ...
                          style: OutlinedButton.styleFrom(
                            backgroundColor: AppColors.kgreenCard.withOpacity(0.4),
                            side: BorderSide(color: AppColors.kgreenCard),
                            shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(Get.width * 0.02),
                            ),
                            padding:
                                EdgeInsets.symmetric(horizontal: Get.width * 0.03, vertical: Get.width * 0.005),
                          ),
                          child: Text("IN PROGRESS",
                              style: AppTypography.kLight14.copyWith(color: AppColors.kgreenCard)),
                        ),
                    )
                  ],
                ),
  ...
  child: ListView.builder(
    ...
    padding: EdgeInsets.symmetric(horizontal: Get.width * 0.04),
    itemBuilder: (context, index) {
      final data = shiftData[index];
      final status = data['status'];

      final shiftCard = buildInProgressShiftUserCard(
        name: data['name'],
        role: data['role'],
        inTime: data['inTime'],
      ...
      if (showAttendance && status == "CLOCK IN AWAITING") {
        return GestureDetector(
          onTap: () {
            Get.to(() => ApproveClockIn());
          },
          child: shiftCard,
        );
      }

      return showAttendance ? shiftCard : buildInProgressNotesCard();
    },
  ),
),
